...
* [Enum.3: `enum` 보다 `enum class`를 사용하라](#Renum-class)
...
### <a name="Renum-class"></a>Enum.3: enum 보다 enum class를 사용하라

##### Reason

의도하지 않은 사용 최소화: 예전의 enum은 너무 쉽게 int 로 변환되었다.
...
##### Example
    ...
    // 아래 두 호출 중 적어도 하나는 버그가 있을 수 있다.
    Print_color(webby);
    Print_color(Product_info::blue);

대신 `enum class`를 사용해라:
    ...
    Web_color webby = Web_color::blue;
    Print_color(webby);  // 오류: Web_color를 int로 변환불가.
    Print_color(Product_info::red);  // 오류: Product_info를 int로 변환불가.
